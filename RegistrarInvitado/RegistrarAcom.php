...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $telefonoAcom = $_POST["telefono"];
    $correo = $_POST["correo"];
    $empresaAcom = $_POST["empresa"];
    ...
    //Para checar si el usuario ya existe
    $sql = "SELECT * FROM usuario WHERE correo ='$correo'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        //Si sí, hacemos un update
        $sql = "UPDATE usuario SET nombre = '$nombre', apellido_paterno = '$apaterno', apellido_materno = '$amaterno', empresa = '$empresaAcom', fotografia = '$fotografiaContenido', telefono = '$telefonoAcom', permisos = '$permisos' WHERE correo = '$correo'";
        if ($conn->query($sql) === TRUE) {
            //update en tabla acompañante
            $sql = "UPDATE acompañante SET tipoIdentificacion = '$documento', id_Invitado = '$id_Invitado', fecha = '$fecha', horaInicio = '$horaInicio' WHERE correo = '$correo'";
            if ($conn->query($sql) === TRUE) {
                //Obtenemos el id del acompañante
                $sql = "SELECT id_Acompañante FROM acompañante WHERE correo = '$correo'";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    $row = $result->fetch_assoc();
                    $id_acompa = $row['id_Acompañante'];
                } else {
                    echo json_encode(array("error" => "No se pudo realizar la actualización"));
                    exit();
                }
                $aux = 1;
            } else {
        ...
        //hacemos insert
        $sql = "INSERT INTO usuario (correo, nombre, apellido_paterno, apellido_materno, empresa, fotografia, telefono, permisos) VALUES ('$correo', '$nombre', '$apaterno', '$amaterno', '$empresaAcom', '$fotografiaContenido', '$telefonoAcom', '$permisos')";
        if ($conn->query($sql) === TRUE) {
