...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $correo = $data["correo"];
    $horaInicioFront = $data["hora_inicio"];
    $fechaFront = $data["fecha"];
    $salaFront = $data["sala"];
    ...
    $eq = "SELECT NoSerie, modelo FROM dispositivo WHERE id_Invitado = '$id_invitado' AND fecha = '$fechaFront' AND horaInicio = '$horaInicioFront'";
    $vehi = "SELECT placa, color, modelo FROM auto WHERE id_Invitado = '$id_invitado' AND fecha = '$fechaFront' AND horaInicio = '$horaInicioFront'";
    $col = "SELECT id_Acompañante, correo FROM acompañante WHERE id_Invitado = '$id_invitado' AND fecha = '$fechaFront' AND horaInicio = '$horaInicioFront'";
    ...
    if (mysqli_num_rows($eq_res) > 0) {
        $equipos = [];
        while ($fila = mysqli_fetch_assoc($eq_res)) {
            $equipos[] = $fila; // Agrega cada fila al array $equipos
        }
        // $compu = "Si";
    } else {
        // $compu = "No";
